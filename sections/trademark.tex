...
\begin{enumerate}
    ...
    See Lanham Act \S\ 33(a), 15 U.S.C. \S\ 1115(a), (b).
    \item Justice Stevens, dissenting: the mark was ``inherently 
    unregistrable,'' so it should not be incontestable.\footnote{Casebook p. 
    865.}
\end{enumerate}

\subsection{Infringement}

\subsubsection{Use as a Trademark: \emph{Rescuecom Corp. v. Google, Inc.}}

\begin{enumerate}
    \item Google recommended ``Rescuecom'' as an AdWords keyword to 
    Rescuecom's competitors.\footnote{Casebook pp. 868--69.}
    \item The district court held that this was an ``internal use,'' not a 
    ``use in commerce.''\footnote{Casebook p. 870.}
    \item Here, the court held that Google's use was a ``use in commerce'' 
    because it ``displays, offers, and sells Rescuecom's mark to Google's 
    advertising customers~.~.~.~.''\footnote{Casebook p. 872.}
\end{enumerate}
...
\begin{enumerate}
    \item Is ``Sleekcraft'' likely to be confused with ``Slickcraft''?
    \item After comparing the marks across eight factors,\footnote{Casebook p. 
    878--80} the court held that confusion was likely and issued an 
    injunction.
\end{enumerate}
